...
export default function App() {
        ...
        (snap) => {
          if (snap.exists()) {
            ...
            if (!remoteData.level || remoteData.level < 5) {
              ...
              setCharacterData(remoteData);
            }
          } else {
            // First boot: populate database with current data
            const currentData = cached ? JSON.parse(cached) : defaultCharacterData;
            const uploadData = (currentData.level && currentData.level >= 5) ? currentData : defaultCharacterData;
  ...
  const saveCharacterData = async (newData: CharacterData) => {
    setCharacterData(newData);
    localStorage.setItem("percival_character_sheet_v5", JSON.stringify(newData));

    if (isFirebaseConfigured && db) {
      try {
        await setDoc(doc(db, "characters", "percival"), newData);
      } catch (err) {
  ...
  const handleUpdateResource = (key: "layOnHands" | "divineSense", amount: number) => {
    const updated = {
      ...characterData,
      [key]: {
        ...characterData[key],
  ...
  return (
    <div className="h-full flex flex-col bg-fantasy-slate-900 text-gray-100 antialiased overflow-hidden select-none relative">
      ...
      <header className="sticky top-0 z-45 bg-slate-900/75 backdrop-blur-xl border-b border-white/5 p-4 shadow-xl flex flex-col gap-3">
        <div className="flex justify-between items-center gap-2">
          {/* Character Badge ID */}
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-fantasy-gold to-fantasy-crimson p-[1.5px] shadow-lg flex items-center justify-center font-display text-xl text-white">
          ...
          <div className="flex items-center gap-2">
            ...
            {isFirebaseConfigured && (
              <span className="w-2 rounded-full h-2 bg-emerald-500 animate-pulse relative mr-1" />
            )}
            
            <button
              onClick={() => setShowLongRestConfirm(true)}
              className="px-3 py-2 bg-gradient-to-r from-fantasy-crimson via-red-800 to-red-950 active:scale-95 text-white text-[10px] font-bold font-mono tracking-widest rounded-lg border border-red-500/20 hover:border-red-500/40 shadow-md shadow-red-950/40 flex items-center gap-1.5 transition-all uppercase"
      ...
      <nav className="sticky bottom-0 left-0 right-0 z-40 bg-slate-900/80 backdrop-blur-2xl border-t border-white/10 p-2 shadow-2xl flex justify-around items-center h-16">
        <button
          onClick={() => setActiveTab("combat")}
          className={`relative p-2 flex flex-col items-center justify-center flex-1 transition-all ${
            activeTab === "combat" ? "text-fantasy-crimson-light scale-110 font-bold" : "text-gray-400 font-normal"
          }`}
          id="tab-combat"
        >
          <Swords className="w-5 h-5 animate-pulse" />
          <span className="text-[10px] font-medium tracking-wide mt-1">Combate</span>
          {activeTab === "combat" && (
            <motion.div
              layoutId="active-tab-glow"
              className="absolute -top-2 w-10 h-0.5 bg-fantasy-crimson rounded-full shadow-[0_0_8px_rgba(239,68,68,0.8)]"
            />
          )}
        </button>
        ...
        <button
          ...
          className={`relative p-2 flex flex-col items-center justify-center flex-1 transition-all ${
            activeTab === "features" ? "text-fantasy-crimson-light scale-110 font-bold" : "text-gray-400"
          }`}
          id="tab-traits"
        >
          <Award className="w-5 h-5" />
          <span className="text-[10px] font-medium tracking-wide mt-1">Traços</span>
          {activeTab === "features" && (
            <motion.div
              layoutId="active-tab-glow"
              className="absolute -top-2 w-10 h-0.5 bg-fantasy-crimson rounded-full shadow-[0_0_8px_rgba(239,68,68,0.8)]"
        ...
        <button
          ...
          className={`relative p-2 flex flex-col items-center justify-center flex-1 transition-all ${
            activeTab === "spells" ? "text-fantasy-crimson-light scale-110 font-bold" : "text-gray-400"
          }`}
          id="tab-spells"
        >
          <Sparkles className="w-5 h-5" />
          <span className="text-[10px] font-medium tracking-wide mt-1">Magias</span>
          {activeTab === "spells" && (
            <motion.div
              layoutId="active-tab-glow"
              className="absolute -top-2 w-10 h-0.5 bg-fantasy-crimson rounded-full shadow-[0_0_8px_rgba(239,68,68,0.8)]"
        ...
        <button
          ...
          className={`relative p-2 flex flex-col items-center justify-center flex-1 transition-all ${
            activeTab === "inventory" ? "text-fantasy-crimson-light scale-110 font-bold" : "text-gray-400"
          }`}
          id="tab-inventory"
        >
          <ShoppingBag className="w-5 h-5" />
          <span className="text-[10px] font-medium tracking-wide mt-1">Mochila</span>
          {activeTab === "inventory" && (
            <motion.div
              layoutId="active-tab-glow"
              className="absolute -top-2 w-10 h-0.5 bg-fantasy-crimson rounded-full shadow-[0_0_8px_rgba(239,68,68,0.8)]"
